    ...
    public function run(): void
{
    ...
    // 2. Buat Admin (Gunakan updateOrCreate biar anti-duplicate)
    ...
    // 3. Buat 5 User Random (Gunakan unique() agar tidak nabrak)
    for ($i = 0; $i < 5; $i++) {
        User::create([
            'username' => fake()->unique()->userName() . rand(100, 999),
            'email' => fake()->unique()->safeEmail(),
            'password' => Hash::make('password123'),
        ]);
    }

    // 4. Buat 5 Kategori Saja
    $categoryNames = ['Action', 'Drama', 'Horror', 'Sci-Fi', 'Animation'];
    foreach ($categoryNames as $name) {
        Category::updateOrCreate(['category_name' => $name]);
    }

    // 5. Ambil ID yang sudah ada untuk relasi
    $catIds = Category::pluck('category_id')->toArray();
    
    // 6. Buat 10 Director & 10 Actor saja (biar enteng)
    Director::factory(10)->create();
    Actor::factory(10)->create();
    $dirIds = Director::pluck('director_id')->toArray();

    // 7. Masukkan 15 Film (Hanya kolom wajib dulu biar nggak berat)
    $moviesData = [
    ...
    foreach ($moviesData as $m) {
        ...
        // 8. Tambahkan Aktor ke Film (Hubungkan tabel pivot)
        $movie->actors()->attach(
            Actor::all()->random(3)->pluck('actor_id')->toArray()
        );

        // 9. Tambahkan Review dari Admin
        Review::create([
    ...
    // 10. Tambahkan 3 film ke Watchlist Admin
    $admin->watchlistMovies()->attach(
        Movie::all()->random(3)->pluck('movie_id')->toArray()
    );
